//! `archcode completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    let mut cmd = PagerArgs::command().name(xai_grok_version::ARCH_CLI_NAME);
    if shell != Shell::Zsh {
        generate(
            shell,
            &mut cmd,
    ...
    let mut buf = Vec::new();
    generate(shell, &mut cmd, xai_grok_version::ARCH_CLI_NAME, &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `archcode worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:archcode-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:archcode-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    // so `archcode worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
